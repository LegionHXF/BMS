...
static void BMS_AnalysisTaskEntry(void *paramter)
{
	BMS_AnalysisCapAndSocInit();  // 初始化  SOC等的初始值的定义 
	while(1)
	{			
		BMS_AnalysisEasy();   // 计算电池的简单分析数据 。 这些数据本身已经存在无需做采样等  
		BMS_AnalysisCalCap();  // 容量温度校准 	实际的电量 
		BMS_AnalysisSocCheck();   //SOC的检查 用算法 计算SOC 
...
// 实时校准容量涉及因素:温度、完整充放电、老化等等  这里只做了温度的校准 ， 计算电池的的额定电量在不同温度下的实际电量
static void BMS_AnalysisCalCap(void)
{	
	BMS_AnalysisTempCal();
}
...
static void BMS_AnalysisAHSocCalculate(void)
{
	...
	/**
	 * @brief 在待机（无充电/放电）状态下，用电压估算SOC：

			  若电池最低电压 ≥ 过压保护阈值 → 满电，SOC = 1；

			  若电池最低电压 ≤ 欠压保护阈值 → 没电，SOC = 0。
	 * 
	 */
	...
	/**
	 * @brief 如果系统处于充电状态，就将测得的电流（安时）累加到剩余容量 CapacityRemain；

			  如果累加后超过电池最大实际容量 CapacityReal，就设为 CapacityReal，防止超充。

			  如果系统处于放电状态，就将当前测得的电流（安时）从剩余容量中减去；

			  如果减去后变为负数，就设为0，防止负容量。
	 * 			
	 */	
...
static void BMS_AnalysisSocCheck(void)
{
	BMS_AnalysisOcvSocCalculate();   // 开路电压法  静态的查表法  

	BMS_AnalysisAHSocCalculate();   // 安时积分法
}
