 ...
 * Change Logs:
 * Date           Author            Notes
 * 2024-6-5       郝小锋          the first version
 * 2024-6-18      郝小锋          (1)重新对三元锂电池SOC对应电压数据表排序
 *									(3)utils增加了二分查找算法用于开路SOC计算
 *									(2)修改了开路电压计算函数使之过程更清晰
 * 2024-7-2       郝小锋          去掉查表方法,更新热敏电阻计算公式
 * 2025-1-5       郝小锋          去掉了BQ769X0驱动的动态内存分配
 */
...
// OCD_THRESH_17mV_8mV  1.6A   OCD_THRESH_22mV_11mV
...
{  	// BQ769X0_ReadBlockWithCRC 上层驱动 Bq的驱动  
	...
	msg[1].buf = readData;
	msg[1].tLen = length * 2;
    // I2C_TransferMessages 底层驱动（我们不用写的）  I2C的驱动  
    if (I2C_TransferMessages(&i2c1, msg, 2) != 2)
    {
